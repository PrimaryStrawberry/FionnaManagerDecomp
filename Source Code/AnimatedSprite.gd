...
onready var burstbounce = get_node("Bust/Bounce")


onready var curseicon = $"/root/Node2D/Fionna/BoobCurse"
...
onready var musclecursedelay = get_node("MuscleCursePlayer/MuscleCurseDelay")
...
func _on_GrowthPeak_timeout()->void :
	...
	if get_node("Missions").boobmission == 1:
		get_node("Bust").bust += 2
		$"/root/Node2D/Sounds/S_SizeUp".playing = true
		print(curse_chance)
		
		if curse_chance == 2:
			get_node("Bust").bust += 5
			$"/root/Node2D/Sounds/S_SizeUp".playing = true
			curse_boob == 1
			cursetimer.start()
			print("CURSED!!")
...
func _on_CurseTimer_timeout()->void :
	...
	if get_node("Bust").bust == (bustthresholds[1 - 2] - 1):
		get_node("Bust").bust += 0.5
		burstbounce.play("burstbounce")
		$"/root/Node2D/Sounds/S_SizeUp".playing = true
		$"/root/Node2D/Sounds/S_CurseHit".playing = true
		print("Intense Curse!" + str(get_node("Bust").bust))
	else :
		get_node("Bust").bust += 0.5
		burstbounce.play("idlejiggle")
		$"/root/Node2D/Sounds/S_CurseHit".playing = true
		$"/root/Node2D/Sounds/S_CurseHit".pitch_scale = (2 - (get_node("Bust").bust / 20))
		print("You're Cursed" + str(get_node("Bust").bust))
